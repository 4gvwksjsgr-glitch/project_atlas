...
class XlsxTabularImportParser {
  const XlsxTabularImportParser();

  TabularImportSource parse({
    ...
    if (nonEmptySheets.length > TabularImportSource.maxSheets) {
    ...
    return TabularImportSource(
      ...
      sheets: [
        for (final s in nonEmptySheets)
          TabularImportSheet(name: s.name, rowCount: s.dataRows),
      ],
  ...
  static TabularImportTable _sheetToTable(Sheet sheet) {
    if (sheet.maxRows < 1) {
  ...
  static String? _cellToString(CellValue? value) {
    ...
    if (value is FormulaCellValue) {
      // Marker consumed by domain validators as formula error.

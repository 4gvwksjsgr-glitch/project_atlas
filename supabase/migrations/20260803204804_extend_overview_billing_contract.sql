-- =============================================================================
-- Project Atlas — Step 14C-1: extend subscription overview billing contract
-- Additive. DROP + CREATE: append columns 17–32; preserve 1–16 names/types/order.
-- =============================================================================
...
BEGIN
  ...
  -- Client-facing status projection (only free|trialing|active)
  ...
  SELECT
    ...
    v_pay_issue,
    v_linked,
    FALSE, -- can_open_billing_portal constant in 14C-1
    v_sync_pending;
END;
$$;

COMMENT ON FUNCTION public.get_company_subscription_overview(UUID) IS
  'Step 14C-1: overview 14B + billing contract append-only; portal CTA always false.';
